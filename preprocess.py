...
def preprocess():
    ...
    parser.add_argument('--train_path', default='train_data.txt', type=str, help='训练日志存放位置')
    parser.add_argument('--save_path', default='train_data.pkl', type=str, help='tokenize的训练数据集')
    ...
    with open(args.train_path, 'rb') as f:  # 读取训练数据集
    ...
    # 开始进行tokenize,保存所有的对话数据,每条数据的格式为："[CLS]utterance1[SEP]utterance2[SEP]utterance3[SEP]"
